...
def execute(user_id):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS friend (id VARCHAR(255))")
    followList = []
    for data in cursor.execute("SELECT * FROM friend"):
    ...
    for data in followList:
        cursor.execute("INSERT INTO friend (id) VALUES (?)", (data,))
    connection.commit()
    connection.close()
