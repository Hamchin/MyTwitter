...
def execute(name_list, list_name):
    twitter = [MyTwitter.login(name)[0] for name in name_list]
    list_id = MyTwitter.get_list_id(list_name)
    tweet_list = []
    timeline = MyTwitter.get_list_timeline(twitter[0], list_id, 1000)
    for tweet in timeline:
        ...
        and MyTwitter.is_favorited(twitter[0], tweet["id_str"]) == False \
    ...
    for account in twitter:
        MyTwitter.post_favorite(account, tweet["id_str"])
